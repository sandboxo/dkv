...
var (
	ServerCmd = &cobra.Command{
		Use:   "server [start]",
		Short: "Command to operate the DKV server",
		Long: `The subcommands of ` + "`" + `dkv server` + "`" + ` operates the DKV server.
		For example, ` + "`" + `dkv server start` + "`" + ` starts the server, 
		and ` + "`" + `dkv server stop` + "`" + ` stops the server.`,
		Args: cobra.MinimumNArgs(1),
	}

	ServerStartCmd = &cobra.Command{
		Use:   "start",
		Short: "Command to start the DKV server",
		Long:  `Commadn to start the DKV server`,
...
func InitFlags() {
	...
	addStringVarToFlagAndViper(ServerCmd, &discoveryConf, "discovery-service-config", "", "A .ini file for configuring discovery service parameters")
	addStringVarToFlagAndViper(ServerCmd, &statsdAddr, "statsd-addr", "", "StatsD service address in host:port format")
	addDurationVarToFlagAndViper(ServerCmd, &replPollInterval, "repl-poll-interval", 5*time.Second, "Interval used for polling changes from master. Eg., 10s, 5ms, 2h, etc.")
	addStringVarToFlagAndViper(ServerCmd, &dbAccessLog, "access-log", "", "File for logging DKV accesses eg., stdout, stderr, /tmp/access.log")
	addBooleanVarToFlagAndViper(ServerCmd, &verboseLogging, "verbose", false, fmt.Sprintf("Enable verbose logging.\nBy default, only warnings and errors are logged. (default %v)", verboseLogging))
	addUint64VarToFlagAndViper(ServerCmd, &blockCacheSize, "block-cache-size", defBlockCacheSize, "Amount of cache (in bytes) to set aside for data blocks. A value of 0 disables block caching altogether.")
	addStringVarToFlagAndViper(ServerCmd, &dcID, "dc-id", "default", "DC / Availability zone identifier")
	addStringVarToFlagAndViper(ServerCmd, &database, "database", "default", "Database identifier")
	addStringVarToFlagAndViper(ServerCmd, &vBucket, "vBucket", "default", "vBucket identifier")
	addStringVarToFlagAndViper(ServerCmd, &replMasterAddr, "repl-master-addr", "", "Service address of DKV master node for replication")
	addBooleanVarToFlagAndViper(ServerCmd, &disableAutoMasterDisc, "disable-auto-master-disc", true, "Disable automated master discovery. Suggested to set to true until https://github.com/flipkart-incubator/dkv/issues/82 is fixed")
	addBooleanVarToFlagAndViper(ServerCmd, &pprofEnable, "pprof", false, "Enable pprof profiling")
}
...
func setDKVDefaultsForNexusDirs() {
	...
	dbPath := viper.GetString("db-folder")
